...
//=======================================
// ★追加：敵をCollisionへ確実に登録する共通処理
// - Rippa と WingRippa を「同じ当たり判定の仕組み」に揃える（登録漏れ対策）
//=======================================
...
void Stage3::Init()
{
    ...
    //5
    {
        ...
        RegisterEnemyCollision(m_collision, rippa_2); // ★追加
...
void Stage3::Update()
{
    ...
    //=========================================================
    // ★重要：eraseをrange-for中にやると iterator が壊れて例外が出る
    //  - ここは「見た目」変えずに安全化（deadの削除は後段でまとめて行う）
    //=========================================================
    {
        ...
        for (size_t i = 0; i < m_enemies.size(); ++i)
        {
            ...
            if (Rippa* r = dynamic_cast<Rippa*>(enemy))
            {
                r->CheckEnemyCollision(m_enemies);
            }
            ...
            if (m_collision->CheckOverlap(playerAABB, enemyAABB))
            {
                ...
                // ★Rippa限定を撤廃：WingRippaも含め「敵ならダメージ」
