...
void Player::Update(float deltaTime,const std::vector<Platform>& platforms,const std::vector<Enemy>& Enemy)
{
    ...
    {
        float angleRad = atan2(rightStick.y, rightStick.x);
        m_aimDirection = { cos(angleRad), sin(angleRad) };

        m_guideline.SetPos(
            pos.x + m_baseGuidelineLength * 0.5f,
            pos.y,
            pos.z
        );
        m_guideline.SetSize(m_baseGuidelineLength, 20, 0);
        m_guideline.SetPivot(-m_baseGuidelineLength * 0.5f, 0, 0);
    ...
    if (!m_isKnockback)
    {
            ...
            && !m_isRibbonOut)
        {
            if (aiming)
            {
                m_throwDirectionRight = (rightStick.x > 0);
                m_ribbon.Throw(m_aimDirection);
            }
            else
            {
                m_throwDirectionRight = m_isLastRightDirection;
                m_ribbon.Throw(
            ...
            m_isRibbonOut = true;
            m_isThrowAnimLock = true;
        }

        if (m_ribbon.GetState() == Ribbon::State::Returning)
        {
            m_isRibbonOut = false;
            m_isThrowAnimLock = false;
        }
    }
